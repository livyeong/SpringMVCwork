...
public class ServletAppContext implements WebMvcConfigurer {
	...
	/*
	 * @Bean public MapperFactoryBean<MapperInterface> test_mapper(SqlSessionFactory
	 * factory) throws Exception { MapperFactoryBean<MapperInterface> factoryBean =
	 * new MapperFactoryBean<MapperInterface>(MapperInterface.class);
	 * factoryBean.setSqlSessionFactory(factory); return factoryBean; }
	 */

	// 쿼리문 실행을 위한 매퍼 객체. 쿼리문 실행을 위한 객체 (mapper 인터페이스를 주입)
	@Bean
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		reg2.excludePathPatterns("/board/main"); //기본 게시판은 아무나 볼 수 있다.
		
		CheckWriterInterceptor checkWriterInterceptor = new CheckWriterInterceptor(loginUserBean, boardService);
		InterceptorRegistration reg3 = registry.addInterceptor(checkWriterInterceptor);
	    reg3.addPathPatterns("board/modify","/board/delete"); //수정과 삭제요청시 인터셉터 실행
	}
